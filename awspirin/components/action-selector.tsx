...
export function ActionSelector({
  ...
  const totalSelectedActions = Object.values(actions)
    .flat()
    .filter((a) => (a as any).selected).length
  ...
  const getCategorySelectedCount = (resourceId: string, category: string) => {
    return getCategoryActions(resourceId, category).filter((a) => (a as any).selected).length
  }

  const getResourceSelectedCount = (resourceId: string) => {
    return actions[resourceId]?.filter((a) => (a as any).selected).length || 0
  }
                        ...
                        <div key={category} className="mb-4 last:mb-0">
                          ...
                          <div className="ml-4 space-y-2">
                            {categoryActions.map((action) => (
                              ...
                              >
                                <Checkbox
                                  checked={(action as any).selected}
